...
app.get('/api/layer/banjir', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT id, nama_wilayah, kecamatan, kelurahan, deskripsi, jenis_bencana, tingkat_risiko, luas_area, created_at, ST_AsGeoJSON(geom) as geometry FROM daerah_rawan WHERE jenis_bencana ILIKE '%banjir%' AND geom IS NOT NULL`
    );
...
app.get('/api/layer/longsor', async (req, res) => {
  const pool = require('./db/pool');
  try {
    const result = await pool.query(
      `SELECT id, nama_wilayah, kecamatan, kelurahan, deskripsi, jenis_bencana, tingkat_risiko, luas_area, created_at, ST_AsGeoJSON(geom) as geometry FROM daerah_rawan WHERE jenis_bencana ILIKE '%longsor%' AND geom IS NOT NULL`
    );
    const features = result.rows.map(r => ({ 
      type: 'Feature', 
      geometry: JSON.parse(r.geometry), 
